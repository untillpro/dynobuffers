...
`
	...
	// Create new buffer from scheme
	{
		...
		require.Panics(func() { require.Equal("cola", b.Get("name").(string)) })
	...
	// Create from bytes
	{
		...
		require.Equal("cola", b.Get("name").(string))
		require.Equal(float32(0.123), b.Get("price"))
		require.Equal(int32(42), b.Get("quantity"))
		// `unknownField` is set but does not exist in scheme, so it is not written
		require.Nil(b.Get("unknownField"))
		// `weight` field exists but not set
		require.Nil(b.Get("weight"))
	}
	...
	// Modify values
	{
		...
		require.Nil(b.Get("name"))
		require.Equal(float32(0.124), b.Get("price").(float32))
		require.Equal(int32(42), b.Get("quantity").(int32))
	}
	...
	// set untyped int value
	{
		...
		require.Equal(int32(45), actual.(int32))
	}
	...
	// Check HasValue
	{
		...
		b.Set("quantity", nil)
		require.NoError(b.CommitChanges())
		require.True(b.HasValue("price"))
		require.False(b.HasValue("quantity")) // set to nil
		require.False(b.HasValue("name"))     // not set
		require.False(b.HasValue("unknownField"))
	}
	...
	// Set string field to non-string -> error
	{
		...
		require.Error(err)
		require.Nil(bytes)
		require.Error(b.CommitChanges())
	}

	// nil Scheme provided -> panic
	{
		require.Panics(func() { NewBuffer(nil) })
		require.Panics(func() { ReadBuffer([]byte{}, nil) })
	}
	b.Release()
...
func TestWriteNewReadOld(t *testing.T) {
	...
	require.Equal("cola", b1.Get("name"))
	require.Equal(float32(0.123), b1.Get("price"))
	require.Equal(int32(42), b1.Get("quantity"))
	require.Nil(b1.Get("newField"))
	b.Release()
	b1.Release()
	require.Zero(GetObjectsInUse())
}
...
func testFieldValues(t *testing.T, b *Buffer, values ...interface{}) {
	...
	for i, f := range b.Scheme.Fields {
		if f.Ft != FieldTypeObject {
			require.Equal(values[i], b.Get(f.Name), f.Name)
			require.Equal(values[i], b.GetByField(f), f.Name)
		}
		if values[i] != nil {
			require.True(b.HasValue(f.Name), f.Name)
		} else {
			require.False(b.HasValue(f.Name), f.Name)
		}
		if f.IsArray {
			if f.Ft == FieldTypeObject {
				nestedArr := b.Get(f.Name).(*ObjectArray)
				valuesNesteds := values[i].([]interface{})
				require.Len(valuesNesteds, nestedArr.Len)
				require.Equal(nestedArr.Buffer, nestedArr.Value()) // cover
				elementsAmount := 0
				// test using iterator
				for nestedArr.Next() {
				...
				require.Equal(nestedArr.Len, elementsAmount)
			} else {
				...
				case FieldTypeByte:
					...
					require.Equal(values[i], arr.Bytes())
				}
			}
		} else {
			// not array
			...
			if values[i] == nil {
				require.False(okGlobal, f.Name)
			} else {
				require.True(okGlobal, f.Name)
			}
		}
	}
}
...
func TestToJSONBasic(t *testing.T) {
	...
	require.Equal("cola", actual["name"])
	require.Equal(float64(0.123), actual["price"])
	require.Equal(float64(42), actual["quantity"])
...
func TestToJSONMapBasic(t *testing.T) {
	...
	dest = b.ToJSONMap()
	require.Len(dest, 3)
	require.Equal("cola", dest["name"])
	require.Equal(float32(0.123), dest["price"])
	require.Equal(int32(42), dest["quantity"])
...
func TestApplyJSONArrays(t *testing.T) {
	...
	for _, wrong := range wrongs {
		...
		require.Nil(bytes, wrong)
		require.Error(err, wrong)
		require.Nil(nilled)
		if wrong.shouldBeNil {
			require.True(b.IsNil(), wrong.json)
		} else {
			require.False(b.IsNil(), wrong.json)
		}
	}
	...
	for _, json := range jsons {
		...
		require.NoError(err)
		require.Nil(bytes)
		require.Equal(allFields, nilled)
...
func TestApplyJSON(t *testing.T) {
	...
	for desc, js := range jsons {
		...
		require.NoError(err, desc)
		require.Nil(bytes, desc)
		require.Nil(nilled)
	}
...
func TestAllValues(t *testing.T) {
	...
	for n, untypedInt := range untypedInts {
		...
		case FieldTypeInt64:
			require.Equal(int64(untypedInt), b.Get(n).(int64), n)
		case FieldTypeFloat32:
			require.Equal(float32(untypedInt), b.Get(n).(float32), n)
		case FieldTypeFloat64:
			require.Equal(float64(untypedInt), b.Get(n).(float64), n)
		}
		b.Release()
	}
	...
	builderBytes, nilled, err = b.ToBytesNilled()
	require.NoError(err)
	require.Contains(nilled, "nes")
	builderBytes = copyBytes(builderBytes)
	b.Release()
	b = ReadBuffer(builderBytes, s)
	require.Nil(b.Get("nes"))
	b.Release()
	...
	for i, f := range s.Fields {
		...
		require.NoError(err)
		require.Equal([]string{f.Name}, nilled)
		bytes = copyBytes(bytes)
		b.Release()
		b = ReadBuffer(bytes, s)
	...
	require.Nil(builderBytes)
	require.NoError(err)
	require.Equal([]string{"string"}, nilled)
	b.Release()
	b = ReadBuffer(builderBytes, s)
	b.Set("string", []byte{})
	builderBytes, nilled, err = b.ToBytesNilled()
...
func testEmpty(require *require.Assertions, b *Buffer) {
	for _, f := range b.Scheme.Fields {
		require.Nil(b.Get(f.Name), f.Name)
		_, ok := b.GetString(f.Name)
		require.False(ok, f.Name)
...
func TestApplyMap(t *testing.T) {
	...
	b := NewBuffer(s)
	require.NoError(b.ApplyMap(nil))
	bytes, nilled, err := b.ToBytesNilled()
	require.Nil(bytes)
	require.NoError(err)
	require.Nil(nilled)
	...
	for _, wrong := range wrongs {
		...
		if wrong.errorOnApply {
			require.Error(b.ApplyMap(wrong.m))
		} else {
			require.NoError(b.ApplyMap(wrong.m))
		}
		bytes, nilled, err = b.ToBytesNilled()
		require.Nil(bytes)
...
func TestApplyMapArrays(t *testing.T) {
	...
	for _, wrong := range wrongs {
		b.Reset(nil)
		if wrong.errorOnApply {
			require.Error(b.ApplyMap(wrong.m), wrong)
		} else {
			require.NoError(b.ApplyMap(wrong.m), wrong)
		}
		bytes, nilled, err := b.ToBytesNilled()
		require.Nil(bytes, wrong)
		require.Nil(nilled)
		if wrong.errorOnApply {
			require.NoError(err, wrong)
		} else {
			require.Error(err, wrong)
		}
	}
	...
	m = map[string]interface{}{
		...
		"bytes":       []byte{13, 14}, // []interface{}{byte(13), byte(14)} is nonsense
		"bytesBase64": "BQY=",
		"intsObj": []interface{}{
			map[string]interface{}{
				"int": int32(8),
...
func TestToJSONAndToJSONMap(t *testing.T) {
	...
	for _, empty := range empties {
		b.ApplyMap(empty)
		require.Equal([]byte("{}"), b.ToJSON())
		require.Empty(b.ToJSONMap())
	}
	b.Release()
...
func TestSchemeToFromYAML(t *testing.T) {
	...
	require.NoError(err)
	schemeNew := NewScheme()
	require.NoError(yaml.Unmarshal(bytes, &schemeNew))

	require.Equal(schemeRoot.Fields, schemeNew.Fields)
	require.Equal(schemeRoot.FieldsMap, schemeNew.FieldsMap)
	...
	require.Error(err)
	require.Error(yaml.Unmarshal([]byte(y), &schemeNew))
...
func TestNestedBasic(t *testing.T) {
	...
	b = ReadBuffer(bytes, schemeRoot)
	require.Equal("str", b.Get("name"))
	require.Equal(int32(42), b.Get("last"))

	bNested = b.Get("nes").(*Buffer)
	require.Equal(int32(42), bNested.Get("quantity"))
	require.Equal(float32(0.123), bNested.Get("price"))
...
func TestNestedAdvanced(t *testing.T) {
	...
	require.Equal(int32(42), b.Get("last"))
	bNested = b.Get("nes").(*Buffer)
	require.Equal(int32(43), bNested.Get("quantity"))
	require.Equal(float32(0.124), bNested.Get("price"))
...
func TestArraysBasic(t *testing.T) {
	...
	require.Equal(2, longsArr.Len())
	require.Equal(int64(5), longsArr.At(0))
	require.Equal(int64(6), longsArr.At(1))
	require.Panics(func() { longsArr.At(-1) })
	require.Panics(func() { longsArr.At(2) })
	require.Nil(b.GetInt64Array("unexisting"))
	...
	require.Equal(longs, longsActual)
	...
	b = ReadBuffer(bytes, s)
	longsArr = b.GetInt64Array("longs")
	require.Equal(3, longsArr.Len())
	require.Equal(int64(7), longsArr.At(0))
	require.Equal(int64(8), longsArr.At(1))
	require.Equal(int64(9), longsArr.At(2))
...
func TestArrays(t *testing.T) {
	...
	require.Nil(b.GetFloat64Array("doubles"))
	require.Nil(b.GetStringArray("strings"))
	require.Nil(b.GetBoolArray("boolTrues"))
	require.Nil(b.GetByteArray("bytes"))
	...
	for fn, values := range tests {
		for _, value := range values {
			...
			b.Set(fn, value)
			bytes, err := b.ToBytes()
			require.NoError(err, fn, value)
			require.Nil(bytes, fn, value)
			b.Release()
			// append
			b = NewBuffer(s)
			b.Append(fn, value)
...
func TestCopyBytes(t *testing.T) {
	...
	require.Equal("str", b.Get("name"))
	require.Equal(int32(42), b.Get("id"))
	arr := b.Get("longs").([]int64)
	require.Equal(int64(45), arr[0])
	require.Equal(int64(46), arr[1])
	require.Equal(float32(0.123), b.Get("float"))
...
func TestCorrectBytesFromNested(t *testing.T) {
	...
	*/
	...
	require.Equal(int32(42), bNested.Get("fld"))
...
func TestObjectAsBytesInv(t *testing.T) {
	...
	require.True(reflect.DeepEqual(bytesNested, nestedTab.Bytes))
	...
	require.Equal("nes1", byteSliceToString(nestedTab.ByteVector(nestedStrOffset)))
...
func TestIterateFields(t *testing.T) {
	...
	b.IterateFields(nil, func(name string, value interface{}) bool {
		switch name {
		case "double":
			require.Equal(float64(0.125), value)
		case "string":
			require.Equal("str", value)
		case "nested1":
			bNested := value.(*Buffer)
			require.Nil(bNested.Get("price"))
			require.Equal(int32(42), bNested.Get("quantity"))
		default:
			t.Fatal(name)
		}
	...
	b.IterateFields(nil, func(name string, value interface{}) bool {
		...
		case "int":
			require.Equal(int32(43), value)
		case "float":
			require.Equal(float32(0.124), value)
		case "double":
			require.Equal(float64(0.125), value)
		case "byte":
			require.Equal(byte(6), value)
		case "boolTrue":
			require.Equal(true, value)
		case "boolFalse":
			require.Equal(false, value)
		case "nested1":
			bNested := value.(*Buffer)
			require.Equal(float32(0.126), bNested.Get("price"))
			require.Equal(int32(44), bNested.Get("quantity"))
		case "nested2":
			bNested := value.(*Buffer)
			require.Equal(float32(0.127), bNested.Get("price"))
			require.Equal(int32(45), bNested.Get("quantity"))
		default:
			t.Fatal(name)
		}
		delete(fieldsMap, name)
	...
	require.Empty(fieldsMap) // `float` is 3rd according to scheme
	...
	require.Len(fieldsMap, 1)
	require.NotNil(fieldsMap["nil"])
	...
	require.Len(fieldsMap, 1)
	require.NotNil(fieldsMap["nested1"])
...
func TestGetNestedScheme(t *testing.T) {
	...
	require.Equal(bNested, bRoot.GetNestedScheme("nes"))
	require.Nil(bRoot.GetNestedScheme("unknown"))
}
...
func TestPreviousResultDamageOnReuse(t *testing.T) {
	...
	require.NoError(err)
	require.NotEqual(bytes1, bytes1Copy)
...
func TestReset(t *testing.T) {
	...
	require.Equal(int32(2), b.Get("quantity"))
	require.Equal(float32(0.124), b.Get("price"))
	...
	// reset to bytes1
	b.Reset(bytes1)
	require.Equal(int32(1), b.Get("quantity"))
	require.Equal(float32(0.123), b.Get("price"))
	...
	b.Release()
	b = ReadBuffer(bytes1, s)
	require.Equal(int32(5), b.Get("quantity"))
